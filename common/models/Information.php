...
use Yii;
use \common\models\base\Information as BaseInformation;
...
class Information extends BaseInformation
{
	...
	const PATH_PHOTO = '/uploads/photos/information';
	...
	{
		$lastId = Information::find()->select('id')->sort_desc()->scalar() ?: 0;
		return ++$lastId;
	}
	...
	public function generatePhotoName()
	{
		if (self::getIsNewRecord()) {
			return 'information_' . self::getLastId() . '-' . (int)(microtime(true) * (1000)) . '.' . $this->photoFile->extension;
		} else {
			return 'information_' . $this->id . '-' . (int)(microtime(true) * (1000)) . '.' . $this->photoFile->extension;
		}
	}
	...
	public function uploadPhoto()
	{
		if ($this->validate(false)) {
			$path = Information::getPhotoAlias();
			if (!file_exists($path)) {
